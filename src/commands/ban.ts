...
export const execute: Command["execute"] = async (interaction) => {
  ...
  try {
    ...
    console.log(`Usuario ${user.tag} banido por ${interaction.user.tag}. Motivo: ${reason}`);
    await interaction.editReply(`✅ ${user.tag} foi banido. Motivo: ${reason}`);
  } catch (error) {
    console.error("Erro ao executar /ban:", error);
    await interaction.editReply("⚠️ Não consegui banir esse usuário. Tente novamente em instantes.");
  }
};
